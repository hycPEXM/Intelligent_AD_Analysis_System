...
HI_S32 SAMPLE_COMM_VGS_FillRect(VIDEO_FRAME_INFO_S *pstFrmInfo, SAMPLE_RECT_ARRAY_S *pstRect, HI_U32 u32Color)
{
    ...
    CHECK_NULL_PTR(pstFrmInfo);
    CHECK_NULL_PTR(pstRect);
    if (pstRect->u16Num == 0) {
        return s32Ret;
    }
    ...
    for (i = 0; i < pstRect->u16Num; i++) {
        ...
        (HI_VOID)memcpy_s(stVgsAddCover.stQuadRangle.stPoint, sizeof(pstRect->astRect[i].astPoint),
...
HI_S32 SAMPLE_COMM_IVE_ReadFile(IVE_IMAGE_S *pstImg, FILE *pFp)
{
    ...
    switch (pstImg->enType) {
        case IVE_IMAGE_TYPE_U8C1: {
            pU8 = (HI_U8 *)(HI_UINTPTR_T)pstImg->au64VirAddr[0];
            for (y = 0; y < height; y++) {
                if (fread(pU8, width, 1, pFp) != 1) {
        ...
        case IVE_IMAGE_TYPE_YUV420SP: {
            ...
            pU8 = (HI_U8 *)(HI_UINTPTR_T)pstImg->au64VirAddr[1];
            for (y = 0; y < height / 2; y++) { /* y shuold be less than height/2 */
                if (fread(pU8, width, 1, pFp) != 1) {
                    SAMPLE_PRT("Read file fail\n");
                    return HI_FAILURE;
                }
        ...
        case IVE_IMAGE_TYPE_U8C3_PACKAGE: {
            pU8 = (HI_U8 *)(HI_UINTPTR_T)pstImg->au64VirAddr[0];
            for (y = 0; y < height; y++) {
                if (fread(pU8, width * 3, 1, pFp) != 1) { /* width * 3 bytes of each element to read */
                    SAMPLE_PRT("Read file fail\n");
                    return HI_FAILURE;
                }

                pU8 += pstImg->au32Stride[0] * 3; /* next fread addr: pstImg->au32Stride[0] * 3 */
            }
            break;
        }
        case IVE_IMAGE_TYPE_U8C3_PLANAR: {
            for (loop = 0; loop < 3; loop++) { /* loop times: 3 */
                pU8 = (HI_U8 *)(HI_UINTPTR_T)pstImg->au64VirAddr[loop];
                for (y = 0; y < height; y++) {
                    if (fread(pU8, width, 1, pFp) != 1) {
                        SAMPLE_PRT("Read file fail\n");
                        return HI_FAILURE;
        ...
        case IVE_IMAGE_TYPE_U16C1: {
            ...
            for (y = 0; y < height; y++) {
                ...
                pU8 += pstImg->au32Stride[0] * 2; /* next fread addr: pstImg->au32Stride[0] * 2 */
            }
            break;
        }
        default:
...
HI_S32 SAMPLE_COMM_IVE_WriteFile(IVE_IMAGE_S *pstImg, FILE *pFp)
{
    ...
    switch (pstImg->enType) {
        ...
        case IVE_IMAGE_TYPE_YUV420SP: {
            ...
            pU8 = SAMPLE_COMM_IVE_CONVERT_64BIT_ADDR(HI_U8, pstImg->au64VirAddr[1]);
            for (y = 0; y < height / 2; y++) { /* y shuold be less than height / 2 */
                if (width != fwrite(pU8, 1, width, pFp)) {
                    SAMPLE_PRT("Write file fail\n");
                    return HI_FAILURE;
        ...
        case IVE_IMAGE_TYPE_U32C1: {
            ...
            for (y = 0; y < height; y++) {
                ...
                pU8 += pstImg->au32Stride[0] * 4; /* next fread addr: pstImg->au32Stride[0] * 4 */
            }
            break;
        }
        default:
    ...
    HI_U16 u16AreaThrStep, HI_U32 u32SrcWidth, HI_U32 u32SrcHeight, HI_U32 u32DstWidth, HI_U32 u32DstHeight)
{
    ...
    if (pstBlob->u8RegionNum > u16RectMaxNum) {
        ...
        do {
            ...
            for (i = 0; i < 254; i++) { /* max region index: 254 */
    ...
    for (i = 0; i < 254; i++) { /* max region index: 254 */
        if (pstBlob->astRegion[i].u32Area > u16Thr) {
            pstRect->astRect[u16Num].astPoint[0].s32X =
                (HI_U32)((HI_FLOAT)pstBlob->astRegion[i].u16Left / (HI_FLOAT)u32SrcWidth * (HI_FLOAT)u32DstWidth) &
                (~1);
            pstRect->astRect[u16Num].astPoint[0].s32Y =
                (HI_U32)((HI_FLOAT)pstBlob->astRegion[i].u16Top / (HI_FLOAT)u32SrcHeight * (HI_FLOAT)u32DstHeight) &
                (~1);
            /* point index: 1 */
            pstRect->astRect[u16Num].astPoint[1].s32X =
                (HI_U32)((HI_FLOAT)pstBlob->astRegion[i].u16Right / (HI_FLOAT)u32SrcWidth * (HI_FLOAT)u32DstWidth) &
                (~1);
            /* point index: 1 */
            pstRect->astRect[u16Num].astPoint[1].s32Y =
                (HI_U32)((HI_FLOAT)pstBlob->astRegion[i].u16Top / (HI_FLOAT)u32SrcHeight * (HI_FLOAT)u32DstHeight) &
                (~1);
            /* point index: 2 */
            pstRect->astRect[u16Num].astPoint[2].s32X =
                (HI_U32)((HI_FLOAT)pstBlob->astRegion[i].u16Right / (HI_FLOAT)u32SrcWidth * (HI_FLOAT)u32DstWidth) &
                (~1);
            /* point index: 2 */
            pstRect->astRect[u16Num].astPoint[2].s32Y =
                (HI_U32)((HI_FLOAT)pstBlob->astRegion[i].u16Bottom / (HI_FLOAT)u32SrcHeight * (HI_FLOAT)u32DstHeight) &
                (~1);
            /* point index: 3 */
            pstRect->astRect[u16Num].astPoint[3].s32X =
                (HI_U32)((HI_FLOAT)pstBlob->astRegion[i].u16Left / (HI_FLOAT)u32SrcWidth * (HI_FLOAT)u32DstWidth) &
                (~1);
            /* point index: 3 */
            pstRect->astRect[u16Num].astPoint[3].s32Y =
                (HI_U32)((HI_FLOAT)pstBlob->astRegion[i].u16Bottom / (HI_FLOAT)u32SrcHeight * (HI_FLOAT)u32DstHeight) &
                (~1);

            bValid = HI_TRUE;
            for (j = 0; j < 3; j++) { /* max j value: 3 */
                for (k = j + 1; k < 4; k++) { /* max k value: 4 */
...
HI_S32 SAMPLE_COMM_IVE_CreateImage(IVE_IMAGE_S *pstImg, IVE_IMAGE_TYPE_E enType, HI_U32 u32Width, HI_U32 u32Height)
{
    ...
    switch (enType) {
        ...
        case IVE_IMAGE_TYPE_U8C3_PACKAGE: {
            ...
            pstImg->au64VirAddr[1] = pstImg->au64VirAddr[0] + 1; /* virAddr index : 1 */
            pstImg->au64VirAddr[2] = pstImg->au64VirAddr[1] + 1; /* virAddr index : 2 */
            pstImg->au64PhyAddr[1] = pstImg->au64PhyAddr[0] + 1; /* phyAddr index : 1 */
    ...
    HI_U32 u32Height)
{
    ...
    switch (enType) {
        ...
        case IVE_IMAGE_TYPE_S8C1: {
            u32Size = pstImg->au32Stride[0] * pstImg->u32Height;
            s32Ret = HI_MPI_SYS_MmzAlloc_Cached(&pstImg->au64PhyAddr[0], (HI_VOID **)&pstImg->au64VirAddr[0], NULL,
                HI_NULL, u32Size);
            if (s32Ret != HI_SUCCESS) {
                SAMPLE_PRT("Mmz Alloc fail,Error(%#x)\n", s32Ret);
                return s32Ret;
...
HI_S32 SAMPLE_COMM_IVE_VbInit(PIC_SIZE_E *paenSize, SIZE_S *pastSize, HI_U32 u32VpssChnNum)
{
    ...
    CHECK_NULL_PTR(paenSize);
    CHECK_NULL_PTR(pastSize);
    ...
    for (i = 0; i < u32VpssChnNum; i++) {
        ...
        SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, VB_FAIL_0, "SAMPLE_COMM_SYS_GetPicSize failed,Error(%#x)!\n",
            s32Ret);

        u64BlkSize = COMMON_GetPicBufferSize(pastSize[i].u32Width, pastSize[i].u32Height, SAMPLE_PIXEL_FORMAT,
    ...
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, VB_FAIL_1, "SAMPLE_COMM_SYS_Init failed,Error(%#x)!\n", s32Ret);
...
HI_S32 SAMPLE_COMM_IVE_DmaImage(VIDEO_FRAME_INFO_S *pstFrameInfo, IVE_DST_IMAGE_S *pstDst, HI_BOOL bInstant)
{
    ...
    IVE_DMA_CTRL_S stCtrl = { IVE_DMA_MODE_DIRECT_COPY, 0, 0, 0, 0 };
    HI_BOOL bFinish = HI_FALSE;
    HI_BOOL bBlock = HI_TRUE;

    CHECK_NULL_PTR(pstFrameInfo);
    CHECK_NULL_PTR(pstDst);
    // fill src
...
HI_S32 SAMPLE_COMM_IVE_StartVo(HI_VOID)
{
    ...
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, VO_FAIL_0, "SAMPLE_COMM_VO_StartDev failed,Error(%#x)!\n", s32Ret);

    s32Ret = SAMPLE_COMM_VO_HdmiStart(stVoPubAttr.enIntfSync);
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, VO_FAIL_1, "SAMPLE_COMM_VO_HdmiStart failed,Error(%#x)!\n", s32Ret);
    ...
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, VO_FAIL_2, "SAMPLE_COMM_VO_GetWH failed,Error(%#x)!\n", s32Ret);

    s32Ret = HI_MPI_VO_SetDisplayBufLen(VoLayer, u32DisBufLen);
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, VO_FAIL_2, "HI_MPI_VO_SetDisplayBufLen failed,Error(%#x)!\n", s32Ret);
    ...
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, VO_FAIL_2, "SAMPLE_COMM_VO_StartLayer failed,Error(%#x)!\n", s32Ret);

    s32Ret = SAMPLE_COMM_VO_StartChn(VoLayer, enVoMode);
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, VO_FAIL_3, "SAMPLE_COMM_VO_StartChn failed,Error(%#x)!\n", s32Ret);
    ...
    PIC_SIZE_E *penExtPicSize)
{
    ...
    CHECK_NULL_PTR(pstSwitch);
    CHECK_NULL_PTR(penExtPicSize);
    (HI_VOID)memset_s(pstViConfig, sizeof(*pstViConfig), 0, sizeof(*pstViConfig));
    ...
    pstViConfig->astViInfo[0].stPipeInfo.aPipe[1] = -1; /* pipe index: 1 */
    pstViConfig->astViInfo[0].stPipeInfo.aPipe[2] = -1; /* pipe index: 2 */
    pstViConfig->astViInfo[0].stPipeInfo.aPipe[3] = -1; /* pipe index: 3 */
    ...
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_0, "Error(%#x),SAMPLE_COMM_VI_GetSizeBySensor failed!\n",
        s32Ret);
    aenSize[1] = *penExtPicSize;

    /* step  1: Init vb */
    s32Ret = SAMPLE_COMM_IVE_VbInit(aenSize, astSize, VPSS_CHN_NUM);
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_0, "Error(%#x),SAMPLE_COMM_IVE_VbInit failed!\n", s32Ret);
    /* step 2: Start vi */
    s32Ret = SAMPLE_COMM_VI_SetParam(pstViConfig);
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_1, "Error(%#x),SAMPLE_COMM_VI_SetParam failed!\n", s32Ret);

    s32Ret = SAMPLE_COMM_VI_StartVi(pstViConfig);
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_1, "Error(%#x),SAMPLE_COMM_VI_StartVi failed!\n", s32Ret);
    /* step 3: Start vpss */
    s32Ret = SAMPLE_COMM_IVE_StartVpss(astSize, VPSS_CHN_NUM);
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_2, "Error(%#x),SAMPLE_IVS_StartVpss failed!\n", s32Ret);
    /* step 4: Bind vpss to vi */
    s32Ret = SAMPLE_COMM_VI_Bind_VPSS(ViPipe0, ViChn, VpssGrp);
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_3, "Error(%#x),SAMPLE_COMM_VI_BindVpss failed!\n", s32Ret);
    // Set vi frame
    s32Ret = HI_MPI_VI_GetChnAttr(ViPipe0, ViChn, &stViChnAttr);
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_4, "Error(%#x),HI_MPI_VI_GetChnAttr failed!\n", s32Ret);

    s32Ret = HI_MPI_VI_SetChnAttr(ViPipe0, ViChn, &stViChnAttr);
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_4, "Error(%#x),HI_MPI_VI_SetChnAttr failed!\n", s32Ret);
    /* step 5: Start Vo */
    if (pstSwitch->bVo == HI_TRUE) {
        s32Ret = SAMPLE_COMM_IVE_StartVo();
        SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_4, "Error(%#x),SAMPLE_COMM_IVE_StartVo failed!\n",
            s32Ret);
    }
    /* step 6: Start Venc */
    if (pstSwitch->bVenc == HI_TRUE) {
        s32Ret = SAMPLE_COMM_VENC_GetGopAttr(VENC_GOPMODE_NORMALP, &stGopAttr);
        SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_5, "Error(%#x),SAMPLE_COMM_VENC_GetGopAttr failed!\n",
            s32Ret);
        s32Ret = SAMPLE_COMM_VENC_Start(VeH264Chn, enStreamType, aenSize[0], enRcMode, 0, bRcnRefShareBuf, &stGopAttr);
        SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_5, "Error(%#x),SAMPLE_COMM_VENC_Start failed!\n", s32Ret);
        s32Ret = SAMPLE_COMM_VENC_StartGetStream(&VeH264Chn, 1);
        SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_INIT_6, "Error(%#x),SAMPLE_COMM_VENC_StartGetStream failed!\n",
            s32Ret);
    }
...
HI_VOID SAMPLE_COMM_IVE_StopViVpssVencVo(SAMPLE_VI_CONFIG_S *pstViConfig, SAMPLE_IVE_SWITCH_S *pstSwitch)
{
    ...
    SAMPLE_COMM_VI_UnBind_VPSS(pstViConfig->astViInfo[0].stPipeInfo.aPipe[0], pstViConfig->astViInfo[0].stChnInfo.ViChn,
        0);
    SAMPLE_COMM_IVE_StopVpss(VPSS_CHN_NUM);
    SAMPLE_COMM_VI_StopVi(pstViConfig);
    SAMPLE_COMM_SYS_Exit();
    ...
    SAMPLE_IVE_SWITCH_S *pstSwitch,PIC_SIZE_E *penExtPicSize)
{
    ...
    /******************************************
     step  1: Init vb
    ******************************************/
    s32Ret = SAMPLE_COMM_IVE_VbInit_Yolov3(aenSize,astSize,VPSS_CHN_NUM);
    SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, END_INIT_0,
...
* function : Stop Vi/Vpss/Venc/Vo
******************************************************************************/
HI_VOID SAMPLE_COMM_IVE_StopViVpssVencVo_Yolov3(SAMPLE_VI_CONFIG_S *pstViConfig,SAMPLE_IVE_SWITCH_S *pstSwitch)
{
    if (HI_TRUE == pstSwitch->bVenc)
    {
        SAMPLE_COMM_VENC_StopGetStream();
...
/*Yolov2*/
HI_S32 SAMPLE_COMM_IVE_VbInit_Yolov2(PIC_SIZE_E *paenSize, SIZE_S *pastSize, HI_U32 u32VpssChnNum)
{
    HI_S32 s32Ret;
    HI_U32 i;
    HI_U64 u64BlkSize;
...
/*Yolov2*/
HI_S32 SAMPLE_COMM_IVE_VbInit_Yolov2_test(PIC_SIZE_E *paenSize, SIZE_S *pastSize, HI_U32 u32VpssChnNum)
{
    HI_S32 s32Ret;
    HI_U32 i;
    HI_U64 u64BlkSize;
    ...
    // VPSS -- VENC
    u64BlkSize = COMMON_GetPicBufferSize(1920, 1080, SAMPLE_PIXEL_FORMAT,
            DATA_BITWIDTH_8, COMPRESS_MODE_NONE, DEFAULT_ALIGN);
        /* comm video buffer */
    stVbConf.astCommPool[u32VpssChnNum].u64BlkSize = u64BlkSize;
    stVbConf.astCommPool[u32VpssChnNum].u32BlkCnt = 16; /* vb block size: 16 */
...
* function : Start Vi/Vpss/Venc/Vo
******************************************************************************/
HI_S32 SAMPLE_COMM_IVE_StartViVpssVencVo_Yolov2(SAMPLE_VI_CONFIG_S *pstViConfig,
    SAMPLE_IVE_SWITCH_S *pstSwitch,PIC_SIZE_E *penExtPicSize)
{
    SIZE_S astSize[VPSS_CHN_NUM];
    PIC_SIZE_E aenSize[VPSS_CHN_NUM];
    ...
    /******************************************
     step  1: Init vb
    ******************************************/
    s32Ret = SAMPLE_COMM_IVE_VbInit_Yolov2(aenSize,astSize,VPSS_CHN_NUM);
    SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, END_INIT_0,
        "Error(%#x),SAMPLE_COMM_IVE_VbInit failed!\n",s32Ret);
    /******************************************
     step 2: Start vi
...
* function : Start Vi/Vpss/Venc/Vo
******************************************************************************/
HI_S32 SAMPLE_COMM_IVE_StartViVpssVencVo_Yolov2_test(SAMPLE_VI_CONFIG_S *pstViConfig,
    SAMPLE_IVE_SWITCH_S *pstSwitch,PIC_SIZE_E *penExtPicSize)
{
    SIZE_S astSize[VPSS_CHN_NUM];
    PIC_SIZE_E aenSize[VPSS_CHN_NUM];
    ...
    /******************************************
     step  1: Init vb
    ******************************************/
    s32Ret = SAMPLE_COMM_IVE_VbInit_Yolov2_test(aenSize,astSize,VPSS_CHN_NUM);
    SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, END_INIT_0,
        "Error(%#x),SAMPLE_COMM_IVE_VbInit failed!\n",s32Ret);
    /******************************************
     step 2: Start vi
    ...
    ******************************************/
    if (HI_TRUE == pstSwitch->bVenc)
    {
        s32Ret = NNIE_START_RTSP();
        SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, END_INIT_4,
            "Error(%#x),SAMPLE_COMM_IVE_StartVENC failed!\n", s32Ret);
    }
...
END_INIT_6:
    if (HI_TRUE == pstSwitch->bVenc)
    {
        RTSP_COMM_VENC_StopGetStream();
        RTSP_COMM_VENC_Stop(0);
    }
END_INIT_5:
    if (HI_TRUE == pstSwitch->bVo)
    {
...
* function : Stop Vi/Vpss/Venc/Vo
******************************************************************************/
HI_VOID SAMPLE_COMM_IVE_StopViVpssVencVo_Yolov2_test(SAMPLE_VI_CONFIG_S *pstViConfig,SAMPLE_IVE_SWITCH_S *pstSwitch)
{
    if (HI_TRUE == pstSwitch->bVenc)
    {
        RTSP_COMM_VENC_StopGetStream();
...
* function : Stop Vi/Vpss/Venc/Vo
******************************************************************************/
HI_VOID SAMPLE_COMM_IVE_StopViVpssVencVo_Yolov2(SAMPLE_VI_CONFIG_S *pstViConfig,SAMPLE_IVE_SWITCH_S *pstSwitch)
{
    if (HI_TRUE == pstSwitch->bVenc)
    {
        SAMPLE_COMM_VENC_StopGetStream();
